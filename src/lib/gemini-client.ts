...
다음 질문:`;
  ...
  try {
    ...
    const hasApiKeyError = apiKeyExpiredPatterns.some(pattern => 
    ...
    // 일반적인 API 키 관련 에러
    if (errorMessage.includes('API key') || errorMessage.includes('API_KEY') || errorString.includes('API_KEY')) {
    ...
    throw new Error(
      `GEMINI_API_ERROR: 다음 질문 생성 실패: ${errorMessage}`
    );
  }
}
...
JSON만 출력하세요:`;
  ...
  try {
    ...
    throw new Error(
      `GEMINI_API_ERROR: 프로젝트 이름 추천 실패: ${errorMessage}`
    );
  }
}
...
마크다운 형식으로 작성해주세요. 전문적이고 상세하게 작성해주세요.`;
  ...
  try {
    ...
    throw new Error(
      `GEMINI_API_ERROR: 문서 생성 실패: ${errorMessage}`
    );
  }
}
